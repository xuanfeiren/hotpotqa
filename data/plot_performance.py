...
def create_plot(algorithms_data, output_filename, title, xlabel, ylabel, data_dir, legend_loc='best'):
    """
...
def main():
    # ========================================================================
    # CONFIGURATION SECTION - MODIFY THIS FOR YOUR PLOTS
    # ========================================================================
    ...
    algorithms_meta = [
        ("gepa/gepa_batchsize_10", "GEPA", "total_samples"),
        ("openevolve", "OpenEvolve", "total_calls"),
        ("Trace", "POLCA (Ours)", "Update/total_samples"),
    ]
    
    # Visual styles (must match order of algorithms_meta)
    # GEPA: #8c564b, --, D
    # OpenEvolve: #2ca02c, --, s
    # POLCA (Ours) / Priority Search: #d62728, -, o
    colors = ['#8c564b', '#2ca02c', '#d62728']
    linestyles = ['--', '--', '-']
    markers = ['D', 's', 'o']
    ...
    plot_configs = {
        'metric_calls': (
            f'HotpotQA Test Score ({model_name})',
            'Number of Metric Calls',
            'Test Score',
            'hotpotqa_gemini_2_5_flash_lite_calls.pdf'
        ),
        'eval_step': (
            f'HotpotQA Test Score ({model_name})',
            'Evaluation Step',
            'Test Score',
            'hotpotqa_gemini_2_5_flash_lite_eval_step.pdf'
        ),
        'prop_step': (
            f'HotpotQA Test Score ({model_name})',
            'Proposal Step',
            'Test Score',
            'hotpotqa_gemini_2_5_flash_lite_prop_step.pdf'
        ),
        'num_proposals': (
            f'HotpotQA Test Score ({model_name})',
            'Number of Proposals',
            'Test Score',
            'hotpotqa_gemini_2_5_flash_lite_num_proposals.pdf'
        )
    }
    ...
    for x_type, config in plot_configs.items():
        ...
        if algorithms_plot_data:
            # For calls move the legend to the right middle, for eval_step move to upper right
            if x_type == 'metric_calls':
                legend_loc = 'center right'
            elif x_type == 'eval_step':
                legend_loc = 'upper right'
            else:
                legend_loc = 'best'
            create_plot(algorithms_plot_data, filename, title, xlabel, ylabel, data_dir, legend_loc=legend_loc)
